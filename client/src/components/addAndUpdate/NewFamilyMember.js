...
import { addNewfamilyMember } from './../../API/FamilyAPI';
...
class CreateFamily extends Component {
    ...
    upload = async (e) => {
        ...
        const newFamilyMember = {
            familyId: "5e72acaab0e6af0017587027",
            newFamilyMember: "5e72abc0b0e6af0017587026"
        };
        const json = JSON.stringify(newFamilyMember);
        ...
        res = await addNewfamilyMember(this.props.data.user.id, JSON.stringify(newFamilyMember));

        if (res.status === 200) {
            dispatch({ type: "LOADER", payload: { display: false } });
            // dispatch({ type: 'ADDED_MY_RECIPE', payload: { user: res.data } })
            this.setState({ updated: true });
            this.props.history.push('/addfamilymember');
        }
        else if (res.error) {
    ...
    render() {
        ...
        return <PageWrapper>
            <div className='addrecipe-container'>
                ...
                {this.state.update ? <button className="btn btn-danger mt-2 w-100 " onClick={this.delete}>Delete Family</button> : null}
            </div >
        </PageWrapper >
    }
}
